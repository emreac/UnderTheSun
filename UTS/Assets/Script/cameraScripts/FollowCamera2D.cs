...
namespace CustomCamera
{
    ...
    public class FollowCamera2D : MonoBehaviour
    {
        ...
        void Start ()
        {
            Verticalcamera = GetComponent<Camera> ();
            vertExtent = Verticalcamera.orthographicSize;
            horzExtent = vertExtent * Screen.width / Screen.height;
            deltaCenterVec = Verticalcamera.ViewportToWorldPoint (new Vector3 (0.5f, 0.5f, 0))
                - Verticalcamera.ViewportToWorldPoint (new Vector3 (cameraCenterX, cameraCenterY, 0));
        ...
        {
            if (target) {
                Vector3 delta = target.position - Verticalcamera.ViewportToWorldPoint (new Vector3 (cameraCenterX, cameraCenterY, 0));
